...
public class QuestCompleter implements Runnable {
  ...
  @Override
  public void run() {
    // TO DO if it still runs like shit then maybe only process a few players per X
    // ticks rather than the whole server in one go
    for (QPlayer qPlayer : plugin.getPlayerManager().getQPlayers()) {
      ...
      for (QuestProgress questProgress : questProgressFile.getAllQuestProgress()) {
        ...
        for (Task task : quest.getTasks()) {
          ...
          if ((taskProgress = questProgress.getTaskProgress(task.getId())) == null
