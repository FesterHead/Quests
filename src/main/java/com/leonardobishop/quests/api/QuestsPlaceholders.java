...
public class QuestsPlaceholders extends PlaceholderExpansion {
  ...
  public String onPlaceholderRequest(Player p, String params) {
    ...
    if (key[0].equals("all") || key[0].equals("completed") || key[0].equals("completedBefore")
        || key[0].equals("started") || key[0].equals("categories")) {
      if (key.length == 1) {
        switch (key[0]) {
          case "all":
          ...
          case "completed":
            return String
                .valueOf(questP.getQuestProgressFile().getQuestsProgress("completed").size());
          case "completedBefore":
            return String
                .valueOf(questP.getQuestProgressFile().getQuestsProgress("completedBefore").size());
          case "started":
            return String
                .valueOf(questP.getQuestProgressFile().getQuestsProgress("started").size());
          case "categories":
      ...
      if (key[1].equals("list") || key[1].equals("l")) {
        ...
        switch (key[0]) {
          ...
          case "completedBefore":
            ...
            for (Quest qCompletedBefore : questP.getQuestProgressFile()
                .getQuestsProgress("completedBefore")) {
    ...
    if (key[0].startsWith("quest:") || key[0].startsWith("q:")) {
      Quest questId =
          this.plugin.getQuestManager().getQuestById(key[0].substring(key[0].lastIndexOf(":") + 1));

      if (key[1].equals("started") || key[1].equals("s")) {
        if (questId != null
            && questP.getQuestProgressFile().getQuestProgress(questId).isStarted()) {
          return "true";
        }
        return "false";
      }
      if (key[1].equals("completed") || key[1].equals("c")) {
        if (questId != null
            && questP.getQuestProgressFile().getQuestProgress(questId).isCompleted()) {
          return "true";
        }
        return "false";
      }
      if (key[1].equals("completedBefore") || key[1].equals("cB")) {
        if (questId != null
            && questP.getQuestProgressFile().getQuestProgress(questId).isCompletedBefore()) {
          return "true";
        }
        return "false";
      }
      if (key[1].equals("completionDate")) {
        if (questId != null
          ...
          return sdf
              .format(questP.getQuestProgressFile().getQuestProgress(questId).getCompletionDate());
        }
        return "Never";
      }
      if (key[1].equals("cooldown")) {
        if (questId != null
            && questP.getQuestProgressFile().getQuestProgress(questId).isCompleted()) {
          String time = this.plugin.convertToFormat(TimeUnit.SECONDS.convert(
              questP.getQuestProgressFile().getCooldownFor(questId), TimeUnit.MILLISECONDS));
          if (time.startsWith("-")) {
            return "0";
          }
          return time;
      ...
      if (key[1].equals("canAccept")) {
        if (questId != null && questP.getQuestProgressFile()
            .canStartQuest(questId) == QuestStartResult.QUEST_SUCCESS) {
          return "true";
        }
        return "false";
      }
      ...
      if (key[1].startsWith("task") || key[1].startsWith("t")) {
        ...
        if (key[2].equals("progress") || key[2].equals("p")) {
          if (questId == null || questP.getQuestProgressFile().getQuestProgress(questId)
              .getTaskProgress(t[1]).getProgress() == null) {
            return "0";
          }
          return String.valueOf(questP.getQuestProgressFile().getQuestProgress(questId)
              .getTaskProgress(t[1]).getProgress());
        }
        if (key[2].equals("completed") || key[2].equals("c")) {
          if (questId == null || questP.getQuestProgressFile().getQuestProgress(questId)
              .getTaskProgress(t[1]).isCompleted()) {
            return "true";
          }
          return "false";
        }
    ...
    if (key[0].startsWith("category:") || key[0].startsWith("c:")) {
      ...
      Category categoryId = this.plugin.getQuestManager()
          .getCategoryById(key[0].substring(key[0].lastIndexOf(":") + 1));
      if (key.length == 2) {
        switch (key[1]) {
          case "all":
          case "a":
      ...
      if (key[2].equals("list") || key[2].equals("l")) {
        ...
        switch (key[1]) {
          ...
          case "cB":
            List<String> listCompletedBefore = new ArrayList<>();
            for (Quest qCompletedBefore : getCategoryQuests(questP, categoryId,
                "completedBefore")) {
              listCompletedBefore.add(qCompletedBefore.getDisplayNameStripped());
            }
            return String.join(separator, listCompletedBefore);
