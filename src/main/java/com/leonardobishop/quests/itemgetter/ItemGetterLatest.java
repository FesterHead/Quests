...
public class ItemGetterLatest implements ItemGetter {
  ...
  /*
   * supporting: - name - material - lore - enchantments (NamespacedKey) - itemflags - unbreakable -
   * attribute modifier - custom model data
   *
   * requires at least API version 1.14
   */
  @Override
  public ItemStack getItem(String path, ConfigurationSection config, Quests plugin,
      ...
      if (hasAttributeModifiers) {
        for (Map<?, ?> attr : cAttributeModifiers) {
          ...
          if (uuid == null) {
            ...
            modifier =
                new AttributeModifier(uuid, cModifierName, cAmount, operation, equipmentSlot);
          }
    ...
    if (!filters.contains(Filter.ENCHANTMENTS)) {
      if (config.isSet(path + "enchantments")) {
        for (String key : config.getStringList(path + "enchantments")) {
          ...
          try {
            namespacedKey = new NamespacedKey(QuestsAPI.getQuestManager().getPlugin(), ench);
          } catch (Exception e) {
