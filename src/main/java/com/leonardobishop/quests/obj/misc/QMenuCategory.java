...
public class QMenuCategory implements QMenu {
  ...
  public void populate(List<QMenuQuest> menuQuests) {
    ...
    for (QMenuQuest qMenuQuest : menuQuests) {
      if (Options.GUI_HIDE_CATEGORIES_NOPERMISSION.getBooleanValue()
          && Quests.get().getQuestManager().getCategoryById(qMenuQuest.getCategoryName()).isPermissionRequired()) {
  ...
  public Inventory toInventory(int page) {
    ...
    for (int pointer = pageMin; pointer < pageMax; pointer++) {
      if (slotsToMenuQuest.containsKey(pointer)) {
        Category category = Quests.get().getQuestManager()
            .getCategoryById(slotsToMenuQuest.get(pointer).getCategoryName());
        if (category != null) {
